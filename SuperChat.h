...
class Client_Window
{
  ...
  char* username;
 public:
  void GUI_main(chat_client* c); // The handler for the GUI thread
  //void set_chatroom(Chatroom* room);
  void display_Login(chat_client* c); // Displays the login screen
  void display_ChatroomSelect(chat_client* c);
  void display_Chatroom(chat_client* c);
